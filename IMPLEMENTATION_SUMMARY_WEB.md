...
#### 3. Database Schema
- ✅ Task table (id, user_id, description, completed, due_date, timestamps)
...
1. **Repository Pattern** - Database access through session layer
2. **Dependency Injection** - FastAPI dependencies for auth/DB
3. **Custom Hooks** - React state encapsulation
4. **API Client Layer** - Centralized fetch logic with retry
5. **Error Boundaries** - Component-level error isolation
6. **Optimistic UI** - Update UI before API confirmation
